...
<xsl:stylesheet
    ...
    <xsl:template match="dri:referenceSet[@id='aspect.artifactbrowser.CommunityBrowser.referenceSet.community-browser']">
        <!-- New UI: for Communities box -->
        <xsl:choose>
            <xsl:when test="/dri:document/dri:body/dri:div[@n='news']/node()">
                <div rend="container-fluid">
                    <div rend="features-boxed">
                        <div rend="container">
                            <h2 rend="text-left">
                                <xsl:apply-templates select="parent::dri:div[@n='comunity-browser']/dri:head"/>
                            </h2>
                            <div id="{@id}" rend="row justify-content-center features">
                                <xsl:apply-templates mode="custom-community-browser"/>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <xsl:template match="dri:reference" mode="custom-community-browser">
        <xsl:variable name="handle">
            <xsl:call-template name="get-handle-class-from-url">
                <xsl:with-param name="url" select="@url"/>
            </xsl:call-template>
        ...
        <xsl:apply-templates select="document($externalMetadataURL)" mode="custom-community-browser"/>

        <xsl:if test="dri:referenceSet/dri:reference">
            <div id="collapse-{$handle}" rend="sub-tree-wrapper hidden">
                <xsl:apply-templates select="dri:referenceSet[dri:reference[@type = 'DSpace Community']]/dri:reference" mode="custom-community-browser"/>
                <xsl:apply-templates select="dri:referenceSet[dri:reference[@type = 'DSpace Collection']]/dri:reference" mode="custom-community-browser"/>
            </div>
        </xsl:if>
    </xsl:template>
